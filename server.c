...
char* createProgramFolder(const char* folderName) {
  ...
  if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_LOCAL_APPDATA, NULL, 0,
    ...
    snprintf(programFolderPath, MAX_PATH, "%s\\%s", localAppDataPath,
             folderName);

    // Create the directory if it doesn't exist
    if (CreateDirectory(programFolderPath, NULL) ||
...
SOCKET waitForClient(SOCKET server_socket, struct sockaddr_in* client) {
  ...
  SOCKET client_socket =
      accept(server_socket, (struct sockaddr*)client, &client_len);
...
void handleFileTransfer(SOCKET client_socket, const char* folderPath) {
  ...
  if (recv(client_socket, (char*)&header, sizeof(header), 0) <= 0 ||
      header.magic != PROTOCOL_MAGIC || header.type != MSG_TYPE_FILE_INIT) {
  ...
  while (total_received < init_msg.filesize) {
    ...
    int chunk_size = recv(client_socket, chunk_msg.chunk_data,
                          sizeof(chunk_msg.chunk_data), 0);
    if (chunk_size <= 0) {
