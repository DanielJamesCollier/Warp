...
void send_file(SOCKET client_socket, const char* file_path) {
  ...
  filename =
      (filename) ? filename + 1 : file_path;  // Move past the slash or use the
  ...
  rewind(file);
  strncpy(init_msg.filename, filename,
          sizeof(init_msg.filename) -
              1);  // Only copy the filename, not the full path
  init_msg.filename[sizeof(init_msg.filename) - 1] = '\0';

  // Send ProtocolHeader first, followed by FileInitMessage
  ProtocolHeader header = {PROTOCOL_MAGIC, MSG_TYPE_FILE_INIT,
                           sizeof(init_msg)};
  ...
  size_t bytes_read;
  while ((bytes_read = fread(chunk_msg.chunk_data, 1,
                             sizeof(chunk_msg.chunk_data), file)) > 0) {
    ...
    int amount =
        (int)sizeof(chunk_msg.header) + (int)bytes_read;  // TODO harden.
    send(client_socket, (char*)&chunk_msg, amount, 0);
  }
